...
def Prompt_Genaration(w, storage_cost, max_index_length, filter_type, index=None, useless_indexes=None):
    ...
    budget_str = f"Given a storage budget limit: {w['budget']}MB.\n"
    ...
    task_str = "Task: Please recommend a best index set for the above database, workload and storage budget.\n"
...
def process_requests(api_key, LLM_type, prompt_v1=None, prompt_v2=None, num_requests=5, mode=1):
    def single_request(prompt):
        ...
        original_output = completion.choices[0].message.content  # 原始输出
        extracted_string = None

        # 提取逻辑
        if "**Final Recommended Index Set**" in original_output:
            content = original_output.split("**Final Recommended Index Set**")[1]
            pattern = r'\[([^\]]+)\]'
        ...
        elif "**Final Recommended Set**" in original_output:
            content = original_output.split("**Final Recommended Set**")[1]
            pattern = r'\[([^\]]+)\]'
            match = re.search(pattern, content)
            if match:
                extracted_string = match.group(1)
                ...
                print("未找到方括号里的内容")
                print(original_output)
        elif "Finalize Index Recommendations" in original_output:
            content = original_output.split("Finalize Index Recommendations")[1]
            pattern = r'\[([^\]]+)\]'
            match = re.search(pattern, content)
            if match:
                extracted_string = match.group(1)
                ...
                print("未找到方括号里的内容")
                print(original_output)
        elif "### Output" in original_output:
            content = original_output.split("### Output")[1]
            pattern = r'\[([^\]]+)\]'
            match = re.search(pattern, content)
            if match:
                extracted_string = match.group(1)
                ...
                print("未找到方括号里的内容")
                print(original_output)
        elif "Final Index Set" in original_output:
            content = original_output.split("Final Index Set")[1]
            pattern = r'\[([^\]]+)\]'
            match = re.search(pattern, content)
            if match:
                extracted_string = match.group(1)
                ...
                print("未找到方括号里的内容")
                print(original_output)
        elif "Recommended Index Set" in original_output and "**Final Recommended Index Set**" not in original_output:
            content = original_output.split("Recommended Index Set")[1]
            pattern = r'\[([^\]]+)\]'
        ...
        return original_output, extracted_string  # 返回原始输出和提取后的输出
    ...
    def execute_requests(prompt, num_requests):
        ...
        with ThreadPoolExecutor(max_workers=num_requests) as executor:
            ...
            for future in futures:
                original_output, extracted_string = future.result()
                if original_output and extracted_string:
                    original_results.append(original_output)
                    results.append(extracted_string)
        return original_results, results

    if mode == 1:
        original_results_v1, results_v1 = execute_requests(prompt_v1, num_requests)
        original_results_v2, results_v2 = execute_requests(prompt_v2, num_requests)
        return original_results_v1 + original_results_v2, results_v1 + results_v2
    elif mode == 2:
        original_results_v1, results_v1 = execute_requests(prompt_v1, num_requests)
        return original_results_v1, results_v1
...
if __name__ == '__main__':
    ...
    config_LLM = json.load(open("config_LLM.json"))
    storage_budget = config_LLM['budget']
    max_index_length = config_LLM['max index length']
    init_index_method = config_LLM['init_index_method']
    api_key = config_LLM['api_key']
    LLM_type = config_LLM['LLM_type']
    epoch = config_LLM['epoch']
    ...
    if init_index_method == "LLM":
        # ask LLM
        original_results, results = process_requests(api_key, LLM_type, promt_v1, promt_v2, epoch)
        new_results = []
        ...
        execute_command(command)
        indexes = json.load(open("index.json"))
    elif init_index_method == "Relaxation":
        ...
        print("Init Index Method Out of Range")
        time.sleep(100000)
    # Test Index
    useless_indexes = []
    storage_cost, reward, indexes, indexes_original, useless_indexes = Test_Index(table_kv, column_kv, workload['workload'], storage_budget, indexes, useless_indexes)
    indexes_trace["Init Index Set"] = indexes_original
    print(f"Reward: {reward}")
    print(f"Indexes: {indexes}")
    print(f"Indexes Text: {indexes_original}")
    LLM4IA_result['Init Index Gen'] = dict()
    LLM4IA_result['Init Index Gen']['storage'] = storage_cost
    LLM4IA_result['Init Index Gen']['reward'] = reward
    LLM4IA_result['Init Index Gen']['experience'] = useless_indexes
    LLM4IA_result['Init Index Gen']['indexes'] = indexes_original
    LLM4IA_result['Init Index Gen']['LLM Input'] = promt_v1
    ...
    round = 0
    LLM4IA_result['Plan-based Enhancement'] = dict()
    while True:
        round += 1
        print(f"Round {round}")
        LLM4IA_result['Plan-based Enhancement'][f'round {round}'] = dict()
        ...
        # ask LLM
        original_results, results = process_requests(api_key, LLM_type, promt_v3, promt_v4, epoch)
        new_results = []
        indexes_trace_temp = []
        for result in results:
            result = result.split("; ")
        ...
        storage_cost, reward, indexes, indexes_original, useless_indexes = Test_Index(table_kv, column_kv, workload['workload'],
                                                                        storage_budget, indexes, useless_indexes)
        print(f"Reward: {reward}")
        print(f"Indexes: {indexes}")
        print(f"Indexes Text: {indexes_original}")
        if reward > reward_old:
            ...
            LLM4IA_result['Plan-based Enhancement'][f'round {round}']['storage'] = storage_cost
            LLM4IA_result['Plan-based Enhancement'][f'round {round}']['reward'] = reward
            LLM4IA_result['Plan-based Enhancement'][f'round {round}']['indexes'] = indexes_original
            LLM4IA_result['Plan-based Enhancement'][f'round {round}']['experience'] = useless_indexes
            LLM4IA_result['Plan-based Enhancement'][f'round {round}']['indexes_trace'] = indexes_trace_temp[0:epoch]
            LLM4IA_result['Plan-based Enhancement'][f'round {round}']['flag_trace'] = indexes_flag_trace_temp[0:epoch]
            LLM4IA_result['Plan-based Enhancement'][f'round {round}']['LLM Input'] = promt_v3
            LLM4IA_result['Plan-based Enhancement'][f'round {round}']['LLM Output'] = original_results[0:epoch]
            plan_based_indexes_trace.append(indexes_trace)
            ...
            break
    LLM4IA_result['Plan-based Enhancement']['round num'] = round - 1
    ...
    round = 0
    LLM4IA_result['Data-based Enhancement'] = dict()
    while True:
        round += 1
        print(f"Round {round}")
        LLM4IA_result['Data-based Enhancement'][f'round {round}'] = dict()
        ...
        # ask LLM
        original_results, results = process_requests(api_key, LLM_type, promt_v5, None, epoch+1, 2)
        new_results = []
        indexes_trace_temp = []
        for result in results:
            result = result.split("; ")
        ...
        if reward > reward_old:
            ...
            LLM4IA_result['Data-based Enhancement'][f'round {round}']['storage'] = storage_cost
            LLM4IA_result['Data-based Enhancement'][f'round {round}']['reward'] = reward
            LLM4IA_result['Data-based Enhancement'][f'round {round}']['indexes'] = indexes_original
            LLM4IA_result['Data-based Enhancement'][f'round {round}']['indexes_trace'] = indexes_trace_temp[0:epoch]
            LLM4IA_result['Data-based Enhancement'][f'round {round}']['experience'] = useless_indexes
            LLM4IA_result['Data-based Enhancement'][f'round {round}']['flag_trace'] = indexes_flag_trace_temp[0:epoch]
            LLM4IA_result['Data-based Enhancement'][f'round {round}']['LLM Input'] = promt_v5
            LLM4IA_result['Data-based Enhancement'][f'round {round}']['LLM Output'] = original_results[0:epoch]
            ...
            indexes_original = indexes_original_old
            break
    LLM4IA_result['Data-based Enhancement']['round num'] = round - 1
    end_time_3 = time.time()
    print(f"Total Inference Time: {end_time_3 - start_time}")
    inference_time = end_time_3 - start_time
    inference_time_details = []
    inference_time_details.append(end_time_1 - start_time)
    inference_time_details.append(end_time_2 - end_time_1)
    inference_time_details.append(end_time_3 - end_time_2)
    init_cost, init_cost_sum, index_cost, index_cost_sum, storage_sum, index_storage, index_reward, index_ratio = Analysis_Index(db_connector,
                                                                                                  workload,
                                                                                                  indexes_original)
    LLM4IA_result["Analysis"] = dict()
    # 1 Index Selection Parameter
    LLM4IA_result["Analysis"]["Index Selection Parameter"] = dict()
    LLM4IA_result["Analysis"]["Index Selection Parameter"]["max index length"] = max_index_length
    LLM4IA_result["Analysis"]["Index Selection Parameter"]["storage budget"] = storage_budget
    # 2 Output Index Set
    LLM4IA_result["Analysis"]["Output Index Set"] = indexes_original
    # 3 Cost Reduction
    LLM4IA_result["Analysis"]["Cost Reduction"] = dict()
    LLM4IA_result["Analysis"]["Cost Reduction"]["no index"] = init_cost_sum
    LLM4IA_result["Analysis"]["Cost Reduction"]["with indexes"] = index_cost_sum
    LLM4IA_result["Analysis"]["Cost Reduction"]["cost reduction ratio"] = 100 * (init_cost_sum - index_cost_sum) / init_cost_sum
    # 4 Inference Time
    LLM4IA_result["Analysis"]["Inference Time"] = inference_time
    LLM4IA_result["Analysis"]["Inference Time Details"] = inference_time_details
    # 5 Index Analysis
    LLM4IA_result["Analysis"]["Index Analysis"] = dict()
    LLM4IA_result["Analysis"]["Index Analysis"]["storage"] = index_storage
    LLM4IA_result["Analysis"]["Index Analysis"]["reward"] = index_reward
    # 6 Cost Per Query
    LLM4IA_result["Analysis"]["Cost Per Query"] = dict()
    LLM4IA_result["Analysis"]["Cost Per Query"]["no index"] = init_cost
    LLM4IA_result["Analysis"]["Cost Per Query"]["with indexes"] = index_cost
    json.dump(LLM4IA_result, open("result_LLM4IA.json", 'w'), indent=4)
