...
class PGHypo:
    def __init__(self, config):
        ...
        self.database = config["benchmark_name"]
    ...
    def execute_create_hypo(self, index):
        table = index.split("#")[0]
        columns = index.split("#")[1]
        sql = "SELECT indexrelid FROM hypopg_create_index('CREATE INDEX ON " + table + "(" + columns + ")') ;"
        cur = self.conn.cursor()
        cur.execute(sql)
        rows = cur.fetchall()
        return int(rows[0][0]), table + "#" + columns
    ...
    def get_storage(self, column):
        oid, index = self.execute_create_hypo(column)
        cost = self.get_storage_cost(oid)
